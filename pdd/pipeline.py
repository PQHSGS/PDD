...
class PDDPipeline:
    ...
    def _resolve_checkpoint_subfolder(self, timestamp_str: str) -> str:
        """Find existing matching checkpoint subfolder or create new timestamped subfolder."""
        ...
        if self.cfg.use_checkpoint and os.path.exists(self.cfg.checkpoint_dir):
            ...
            if matching_subdirs:
                # Pick the latest matching subfolder
                matching_subdirs.sort(reverse=True)
                latest_dir = os.path.join(self.cfg.checkpoint_dir, matching_subdirs[0])
                logger.info(f"Resolved existing checkpoint subfolder: '{latest_dir}'")
                return latest_dir
